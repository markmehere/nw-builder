...
function getManifest(manifestUrl) {
  ...
  return get(manifestUrl, { json: true }).then(function (body) {
    body.versions = body.versions.map((version) => {
      if (version.version.indexOf("v0.6") === 0 || version.version.indexOf("v0.7") === 0 || version.version.indexOf("v0.8") === 0) {
        return {
          ...version,
          files: version.files.concat("osx-arm64")
        };
      }
      else {
        return version;
...
function getVersionsFromManifest(downloadUrl, manifestUrl) {
  var mapFilesToPlatforms = function (files) {
    return files.map(function (file) {
      ...
      return file.replace(/-(x|ia)/, "").replace(/-arm64/, "arm");
    });
  };
